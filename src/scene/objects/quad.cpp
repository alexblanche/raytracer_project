...
std::optional<real> quad::measure_distance(const ray& r) const {
    ...
    if (std::abs(det12xy) > 0.00000001f) {
        ...
        if (l1xy >= 0.0_r && l2xy >= 0.0_r && l1xy <= 1.0_r && l1xy + l2xy <= 1.0_r) {
            ...
            if (l1axy >= 0.0_r && l1axy <= 1.0_r) {
                const real l2axy = (v3.x * c.y - v3.y * c.x) / det23xy;
                return (l2axy >= 0.0_r && l1axy + l2axy <= 1.0_r) ?
    ...
    if (std::abs(det12xz) > 0.00000001f) {
        ...
        if (l1xz >= 0.0_r && l2xz >= 0.0_r && l1xz <= 1.0_r && l1xz + l2xz <= 1.0_r) {
            ...
            if (l1axz >= 0.0_r && l1axz <= 1.0_r) {
                const real l2axz = (v3.x * c.z - v3.z * c.x) / det23xz;
                return (l2axz >= 0.0_r && l1axz + l2axz <= 1.0_r) ?
                    std::optional(t) : std::nullopt;
            }
            return std::nullopt;
        }
    ...
    if (std::abs(det12yz) > 0.00000001f) {
        ...
        if (l1yz >= 0.0_r && l2yz >= 0.0_r && l1yz <= 1.0_r && l1yz + l2yz <= 1.0_r) {
            ...
            if (l1ayz >= 0.0_r && l1ayz <= 1.0_r) {
                const real l2ayz = (v3.y * c.z - v3.z * c.y) / det23yz;
                return (l2ayz >= 0.0_r && l1ayz + l2ayz <= 1.0_r) ?
                    std::optional(t) : std::nullopt;
            }
            return std::nullopt;
        }
...
std::optional<real> quad::measure_distance(const ray& r) const {
	...
	const rt::vector c = fma(r.direction, t, r.origin) - position;
