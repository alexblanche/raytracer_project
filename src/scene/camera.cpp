...
ray camera::gen_ray_normal(const int i, const int j, const double& std_dev, randomgen& rg) const {
    std::pair<double, double> shift = rg.random_pair_normal(0, std_dev);
    rt::vector dir = (mhalf_fovw + (((double) i) + shift.first) * di) * to_the_right
        + (mhalf_fovh + (((double) j) + shift.second) * dj) * to_the_bottom
        + distance * direction;
    return ray(origin, dir.unit());
}
