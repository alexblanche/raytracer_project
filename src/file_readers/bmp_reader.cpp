...
bool read_bmp(const char* file_name, std::vector<std::vector<rt::color>>& data) {
    ...
    try {
        ...
        */
        ...
        //bool padding = false;
        unsigned int p = (BYTES_PER_COLOR * bmpwidth) % 4;
        if (p != 0) {
            //padding = true;
            p = 4 - p;
        }
...
bool write_bmp(const char* file_name, const std::vector<std::vector<rt::color>>& data,
        ...
        for (int j = 0; j < height; j++) {
            ...
            for (int i = 0; i < width; i++) {
                ...
                if (gamma_enabled) {
                    ...
                    const unsigned char r = std::min(c.get_red()   * invN, 255.0);
                    const unsigned char g = std::min(c.get_green() * invN, 255.0);
                    const unsigned char b = std::min(c.get_blue()  * invN, 255.0);
                    ret = fprintf(file, "%c%c%c", b, g, r);
                }
                throw_if_error(ret);
            }
