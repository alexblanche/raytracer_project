...
exit_status raw_data::export_data(const std::string& file_name, const image& image, const format format) {
...
static std::unique_ptr<rt::color[]> read_raw_file_content(const file& f, const format format,
...
std::optional<image> raw_data::read_file(const std::string& file_name) {
...
exit_status raw_data::combine_files(const std::string& dest_bmp_name, const std::string& dest_raw_name,
    ...
    try {
        ...
        const exit_status success_bmp = bmp::export_data(dest_bmp_name, image);

        constexpr format DEFAULT_FORMAT = Binary;
        const exit_status success_raw = export_data(dest_raw_name, image, DEFAULT_FORMAT);
