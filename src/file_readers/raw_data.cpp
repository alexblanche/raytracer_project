...
/* Reads a file file_name generated by export_raw, and returns a matrix with its content
   Writes the number of rays in the variable, and writes true in the variable success if the operation was successful */
std::vector<std::vector<rt::color>> read_raw(const char* file_name, unsigned int& number_of_rays, bool& success) {
    ...
    if (file == NULL) {
        ...
        success = false;
        std::vector<std::vector<rt::color>> fail_matrix(1, std::vector<rt::color>(1));
        return fail_matrix;
    }
