...
enum class ray_orientation_type {
    Inward, Outward
};
...
class hit {
    private:
        ...
        ray_orientation_type ray_orientation;
        object_type type;

    public:
        hit(const rt::vector& point, const rt::vector& normal, const object* hit_object,
            const ray_orientation_type ray_orientation, const object_type type)
            : point(point), normal(normal), hit_object(hit_object), ray_orientation(ray_orientation), type(type) {}
        ...
        static inline ray_orientation_type compute_ray_orientation(const rt::vector& direction, const rt::vector& normal) {
        ...
        [[nodiscard]] inline ray_orientation_type get_ray_orientation() const {
            return ray_orientation;
        }

        [[nodiscard]] inline object_type get_object_type() const {
            return type;
        }
        ...
        [[nodiscard]] inline rt::vector biased_point(const ray_orientation_type bias_orientation) const {

            const real bias_norm = (ray_orientation != bias_orientation) ? BIAS_NORM : (-BIAS_NORM);
            return fma(normal, bias_norm, point);
        }

        template <ray_orientation_type ray_orientation, ray_orientation_type bias_orientation>
